...
const LoginPage = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // Mock Google sign in
      await signIn('demo@google.com', 'google-mock');
      router.push('/');
    } catch {
      ...
      <div className="max-w-md w-full space-y-8 p-10 bg-white shadow-lg rounded-xl">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            ...
            <Link href="/register" className="font-medium text-blue-800 hover:text-blue-700">
              Sign up
            </Link>
          </p>
        </div>
        ...
        <div className="flex items-center justify-center">
          ...
          <span className="flex-shrink mx-4 text-sm text-gray-500">Or continue with username/email</span>
          <div className="flex-grow border-t border-gray-300"></div>
        </div>

        <form className="space-y-6" onSubmit={handleSignIn}>
          <div>
            <label htmlFor="email" className="text-sm font-medium text-gray-700">Username or email address</label>
            <input
              id="email"
              type="email"
              required
              ...
              disabled={isLoading}
              className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-800 focus:border-blue-800 sm:text-sm disabled:opacity-50"
            />
          </div>
          <div>
            ...
            <div className="mt-1 relative">
              <input
                ...
                disabled={isLoading}
                className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-800 focus:border-blue-800 sm:text-sm disabled:opacity-50"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                disabled={isLoading}
                className="absolute inset-y-0 right-0 pr-3 flex items-center text-sm font-medium text-blue-800 hover:text-blue-700 disabled:opacity-50"
              >
